...
import com.lyyj.activity.demo.gridview.gappsdraggridview.DragLayer;
...
public class DragGridView extends GridView{
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if(isDrag && mDragImageView != null){
			switch(ev.getAction()){
			case MotionEvent.ACTION_MOVE:
            	DragLayer dragLayer1 =(DragLayer)getParent().getParent();
            	Log.e(TAG,"kaka grid actionmove isEditMode():"+"event.getrawX():"+ev.getRawX()+"event.getrawY():"+ev.getRawY()+"dragLayer1:"+dragLayer1);
            	if(dragLayer1!=null&&dragLayer1.isHit((int) ev.getRawX(), (int) ev.getRawY())){
            		Log.e(TAG,"kaka grid action move shortcut");
            		dragLayer1.hitTip();
            	}else{
            		if(dragLayer1!=null) dragLayer1.unHitTip();
            	}
				moveX = (int) ev.getX();
				moveY = (int) ev.getY();
				...
				break;
			case MotionEvent.ACTION_UP:
            	DragLayer dragLayer2 =(DragLayer)getParent().getParent();
            	if(dragLayer2!=null) dragLayer2.unHitTip();
            	Log.e(TAG,"kaka grid action up isEditMode():"+"event.getrawX():"+ev.getRawX()+"event.getrawY():"+ev.getRawY()+"dragLayer2:"+dragLayer2);
            	if(dragLayer2!=null&&dragLayer2.isHit((int) ev.getRawX(), (int) ev.getRawY())){
            		Log.e(TAG,"kaka grid action up shortcut");
                	HashMap<String, Object> map =(HashMap<String, Object>)getAdapter().getItem(mDragPosition);
    ...
    public boolean addShortcut(ResolveInfo resolveInfo) {
        ...
        String label = resolveInfo.loadLabel(pkgMag).toString();
        String mainAct = resolveInfo.activityInfo.name;
        // int iconIdentifier = resolveInfo.activityInfo.applicationInfo.icon;
        //int iconIdentifier = resolveInfo.activityInfo.icon; 
        if(mainAct == null) {
            return false;
        }
        ...
        if(context.getPackageName().equals(pkg)) {
            ...
            try {
                pkgContext = context.createPackageContext(pkg, 0x3);
        ...
        */
        shortcut.putExtra("android.intent.extra.shortcut.ICON",drawableToBitmap(resolveInfo.loadIcon(pkgMag)));
        ...
        View view=inflater.inflate(R.layout.toast_info, null);
     	...
     	img.setImageBitmap(drawableToBitmap(resolveInfo.loadIcon(pkgMag)));
