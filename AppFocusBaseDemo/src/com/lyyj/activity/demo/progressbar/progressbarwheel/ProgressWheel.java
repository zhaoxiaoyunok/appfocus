package com.lyyj.activity.demo.progressbar.progressbarwheel;
...
public class ProgressWheel extends View {
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressWheel_barWidthPbWheel,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_rimWidthPbWheel,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel_spinSpeedPbWheel,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressWheel_delayMillisPbWheel,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressWheel_barColorPbWheel, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressWheel_barLengthPbWheel,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressWheel_textSizePbWheel,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressWheel_textColorPbWheel,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressWheel_textPbWheel)) {
            setText(a.getString(R.styleable.ProgressWheel_textPbWheel));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressWheel_rimColorPbWheel,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressWheel_circleColorPbWheel,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressWheel_contourColorPbWheel, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressWheel_contourSizePbWheel, contourSize);
