...
import com.appfocusbase.login.LoginActivity;
...
//	private Zhao msp;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mAfTitleBar.setTitleText(R.string.app_name);
	...
	public void onBackPressed() {
		if (menu.isMenuShowing()) {
			...
			if (mMainContentFragment.canBack()) {
				if (isExit == false) {
					isExit = true;
					AfToastUtil.showToast(MainActivity.this,"再按一次退出程序");
...
//
//		});
			List<User> paramList =mUserDao.getUserLoginedByUser(user);
	 		if (paramList != null && paramList.size() > 0) {
	 	    //登录IM
	 		loginIMTask((User) paramList.get(0));
	 	}else{
	 		AfToastUtil.showToast(MainActivity.this,"IM信息缺失");
	 	}
	}
...
//	               try{
//	                   //设置用户名与密码
//	                   code = IMUtil.loginIM(user.getName(),user.getPassword());
//	               } catch (Exception e) {
//	                   e.printStackTrace();
//	               }
//	               return code;
