...
public class AbFileDownloader {
	...
	public AbFileDownloader(Context context,DownFile downFile,int threadNum) {
		try {
			...
			// 构建保存文件
			String fileName = AfFileUtil.getRealFileNameFromUrl(mDownFile.getDownUrl());
			saveFile = new File(AfFileUtil.getFileDownloadDir(context)+fileName);
	...
	public static void printResponseHeader(HttpURLConnection http) {
		...
		for (Map.Entry<String, String> entry : header.entrySet()) {
			...
			AfLogUtil.i(AbFileDownloader.class, key + entry.getValue());
		}
	}
