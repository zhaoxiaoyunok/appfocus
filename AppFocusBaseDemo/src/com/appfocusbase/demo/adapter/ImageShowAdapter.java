...
public class ImageShowAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(!AfStrUtil.isEmpty(imagePath)){
		  //从缓存中获取图片，很重要否则会导致页面闪动
      	  Bitmap bitmap = AfImageBaseCache.getInstance().getBitmap(imagePath);
      	  //缓存中没有则从网络和SD卡获取
      	  if(bitmap == null){
	      		...
	      		if(imagePath.indexOf("http://")!=-1){
	               ...
	               // mAbImageLoader.display(holder.mImageView1,imagePath,this.mWidth,this.mHeight);
	                Ion.with(holder.mImageView1).placeholder(R.drawable.progress_loading2).resize(100,100).load(imagePath);
					...
					Bitmap mBitmap = AfFileUtil.getBitmapFromSD(new File(imagePath), AfImageUtil.SCALEIMG, mWidth, mHeight);
					if(mBitmap!=null){
