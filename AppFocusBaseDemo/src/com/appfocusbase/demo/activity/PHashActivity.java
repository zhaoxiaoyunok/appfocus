...
import com.af.view.chart.PointStyle;
import com.af.view.chart.XYMultipleSeriesDataset;
import com.af.view.chart.XYMultipleSeriesRenderer;
import com.af.view.chart.XYSeriesRenderer;
...
public class PHashActivity extends AfActivity {
	...
	private AfResult mAbResult = null;
	...
	private AfTaskPool mAbTaskPool = AfTaskPool.getInstance();
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    if(AfStrUtil.isEmpty(photo_dir)){
	    	AfToastUtil.showToast(PHashActivity.this,"存储卡不存在");
	    }else{
		...
		albumButton.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				AfDialogUtil.removeDialog(PHashActivity.this);
				// 从相册中去获取	
				try {
					...
					AfToastUtil.showToast(PHashActivity.this,"没有找到照片");
				}
			}
		...
		addBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				if(filePathAll==null){
					AfToastUtil.showToast(PHashActivity.this,"请先创建图片索引!");
				}else{
					AfDialogUtil.showDialog(mAvatarView);
				}
		...
		createBtn.setOnClickListener(new OnClickListener(){
			...
			@Override
			public void onClick(View v) {
				AfDialogUtil.showProgressDialog(PHashActivity.this,R.drawable.progress_circular,"正在查询...");
				...
				item.setListener(new AfTaskListener() {
					...
					public void update() {
						AfDialogUtil.removeDialog(PHashActivity.this);
						AfToastUtil.showToast(PHashActivity.this,"创建图片索引完成!");
					}
					...
					public void get() {
			   		    try {
							...
							AfLogUtil.prepareLog(PHashActivity.this);
							//查询手机中所有图片
							...
							if(!AfStrUtil.isEmpty(fileStrs)){
								if(fileStrs.indexOf(",")!=-1){
									String [] paths = fileStrs.split(",");
									for(int i=0;i<paths.length;i++){
										String str = paths[i];
										if(!AfStrUtil.isEmpty(str)){
							...
							for(int i=0;i<files.size();i++){
								File f = files.get(i);
								AfLogUtil.d(PHashActivity.this, "图片的路径是 = " + f.getPath());
								Bitmap bitmap = AfFileUtil.getBitmapFromSD(f);
								if(bitmap==null){
									//图片有问题
									files.remove(i);
									i--;
									...
									//计算hash
									String hashCode = AfImageUtil.getDCTHashCode(bitmap);
									hashCodes.add(hashCode);
									AfLogUtil.d(PHashActivity.this,"hashCodes add:"+i+":"+hashCode);
									//颜色分布
									Bitmap bitmapT = AfImageUtil.getCutBitmap(bitmap,360,360);
									int [] colors = AfImageUtil.getColorHistogram(bitmapT);
									colorHistogram.add(colors);
									AfImageUtil.releaseBitmap(bitmap);
								}
							}
							AfLogUtil.d(PHashActivity.this, "创建索引",true);
			   		    } catch (Exception e) {
			   		    	AfToastUtil.showToastInThread(PHashActivity.this,e.getMessage());
			   		    }
				  };
				});
	...
	private void doPickPhotoAction() {
		...
		if (status.equals(Environment.MEDIA_MOUNTED)) {
			...
			AfToastUtil.showToast(PHashActivity.this,"没有可用的存储卡");
		}
	}
	...
	protected void doTakePhoto() {
		try {
			...
			AfToastUtil.showToast(PHashActivity.this,"未找到系统相机程序");
		}
	}
		...
		switch (requestCode) {
			case PHOTO_PICKED_WITH_DATA:
				...
				if(!AfStrUtil.isEmpty(currentFilePath)){
		        	...
		        	AfToastUtil.showToast(PHashActivity.this,"未在存储卡中找到这个文件");
		        }
				break;
			case CAMERA_WITH_DATA:
				AfLogUtil.d(PHashActivity.this, "将要进行裁剪的图片的路径是 = " + mCurrentPhotoFile.getPath());
			...
			case CAMERA_CROP_DATA:
				...
				AfLogUtil.d(PHashActivity.this, "裁剪后得到的图片的路径是 = " + path);
		    	
		    	Bitmap bitmap = AfFileUtil.getBitmapFromSD(new File(path));
		    	myImage.setImageBitmap(bitmap);
		    	
		    	hashCodesAndDis.clear();
		    	mPhotoList.clear();
		    	mImagePathAdapter.notifyDataSetChanged();
		    	AfViewUtil.setAbsListViewHeight(mGridView,3,25);
		    	
		    	//先过滤颜色相似
		    	//颜色分布
				int [] sourceColors = AfImageUtil.getColorHistogram(bitmap);
		    	
		    	//获取这个图的hashcode
		    	String sourceHashCode = AfImageUtil.getDCTHashCode(bitmap);
		    	AfLogUtil.d(PHashActivity.this,"this image sourceHashCode:"+sourceHashCode);
		    	//计算距离
		    	for(int i = 0;i<hashCodes.size();i++){ 
		    		String hashCode = hashCodes.get(i);
		    		int distanceNew = AfImageUtil.hammingDistance(sourceHashCode, hashCode);
		    		if(distanceNew<5){
                ...
                for(int i=0;i<hashCodesAndDisNew.length;i++){
            	   ...
            	   AfLogUtil.d(PHashActivity.this,"匹配结果:"+pathNew+":"+hashCodesAndDisNew[i].getValue());
	...
	public void createChart(int [] colorsArea,String title){
	    ...
	    values.add(AfMathUtil.intToDoubleArray(colorsArea));
	    ...
	    chartView.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, 250));
