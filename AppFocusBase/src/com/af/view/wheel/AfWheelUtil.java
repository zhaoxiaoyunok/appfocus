...
import com.af.util.AfDateUtil;
import com.af.util.AfDialogUtil;
import com.af.util.AfStrUtil;
import com.af.view.wheel.AfWheelView.AbOnWheelChangedListener;
...
public class AfWheelUtil {
	 ...
	 */
	public static void initWheelDatePicker(final AfActivity activity,final TextView mText,final AfWheelView mWheelViewY,final AfWheelView mWheelViewM,final AfWheelView mWheelViewD,
		...
		mText.setText(AfStrUtil.dateTimeFormat(defaultYear+"-"+defaultMonth+"-"+defaultDay));
		final List<String> list_big = Arrays.asList(months_big);
		final List<String> list_little = Arrays.asList(months_little);
		
		//设置"年"的显示数据
		mWheelViewY.setAdapter(new AfNumericWheelAdapter(startYear, endYear));
		...
		// 月
		mWheelViewM.setAdapter(new AfNumericWheelAdapter(1, 12));
		...
		if (list_big.contains(String.valueOf(month + 1))) {
			mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 31));
		} else if (list_little.contains(String.valueOf(month + 1))) {
			mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 30));
		} else {
			// 闰年
			if (AfDateUtil.isLeapYear(year)){
				mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 29));
			}else{
				mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 28));
			}
		}
		...
		AbOnWheelChangedListener wheelListener_year = new AbOnWheelChangedListener() {
			...
			public void onChanged(AfWheelView wheel, int oldValue, int newValue) {
				...
				if (list_big.contains(String.valueOf(mWheelViewM.getCurrentItem() + 1))) {
					...
					if (AfDateUtil.isLeapYear(year_num))
						mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 29));
					else
						mWheelViewD.setAdapter(new AfNumericWheelAdapter(1, 28));
				}
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				mText.setText(AfStrUtil.dateTimeFormat(year+"-"+month+"-"+day));
			}
	 ...
	 * 
	 */
	public static void initWheelTimePicker(final AfActivity activity,final TextView mText,final AfWheelView mWheelViewMD,final AfWheelView mWheelViewHH,final AfWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		String val = AfStrUtil.dateTimeFormat(defaultYear+"-"+defaultMonth+"-"+defaultDay+" "+defaultHour+":"+defaultMinute+":"+second) ;
		mText.setText(val);
		// 添加大小月月份并将其转换为list,方便之后的判断
		String[] months_big = { "1", "3", "5", "7", "8", "10", "12" };
		String[] months_little = { "4", "6", "9", "11" };
		...
		for(int i=1;i<13;i++){
			if(list_big.contains(String.valueOf(i))){
				...
				if(i==2){
					if(AfDateUtil.isLeapYear(defaultYear)){
						for(int j=1;j<28;j++){
							textDMList.add(i+"月"+" "+j+"日");
							textDMDateList.add(defaultYear+"-"+i+"-"+j);
						}
		...
		// 月日
		mWheelViewMD.setAdapter(new AfStringWheelAdapter(textDMList,AfStrUtil.strLength("12月"+" "+"12日")));
		...
		// 时
		mWheelViewHH.setAdapter(new AfNumericWheelAdapter(0, 23));
		...
		// 分
		mWheelViewMM.setAdapter(new AfNumericWheelAdapter(0, 59));
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				Calendar calendar = Calendar.getInstance();
				int second = calendar.get(Calendar.SECOND);
				String val = AfStrUtil.dateTimeFormat(dmStr+" "+index2+":"+index3+":"+second) ;
				mText.setText(val);
			}
	 ...
	 * @param initStart the init start
	 */
	public static void initWheelTimePicker2(final AfActivity activity,final TextView mText,final AfWheelView mWheelViewHH,final AfWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		String val = AfStrUtil.dateTimeFormat(defaultHour+":"+defaultMinute) ;
		mText.setText(val);
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				int index2 = mWheelViewHH.getCurrentItem();
				int index3 = mWheelViewMM.getCurrentItem();
				String val = AfStrUtil.dateTimeFormat(index2+":"+index3) ;
				mText.setText(val);
			}
