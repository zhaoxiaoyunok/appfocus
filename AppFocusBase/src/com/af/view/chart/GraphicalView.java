...
import com.af.util.AfGraphicUtil;
...
public class GraphicalView extends View {
  ...
  public GraphicalView(Context context, AbstractChart chart) {
    ...
    if (mRenderer.isZoomButtonsVisible()) {
      zoomInImage = AfFileUtil.getBitmapFromSrc("image/zoom_in.png");
      zoomOutImage = AfFileUtil.getBitmapFromSrc("image/zoom_out.png");
      fitZoomImage = AfFileUtil.getBitmapFromSrc("image/zoom-1.png");
    }
    ...
    if (mChart instanceof XYChart) {
        ...
        //按分辨率转换
        mXYMultipleSeriesRenderer.setExplainTextSize1(AfViewUtil.scaleTextValue(mContext, explainTextSize1));
        mXYMultipleSeriesRenderer.setExplainTextSize2(AfViewUtil.scaleTextValue(mContext, explainTextSize2));
        mXYMultipleSeriesRenderer.setScaleCircleRadius(AfViewUtil.scaleValue(mContext, scaleCircleRadius));
        mXYMultipleSeriesRenderer.setScaleRectWidth(AfViewUtil.scaleValue(mContext, scaleRectWidth));
        mXYMultipleSeriesRenderer.setScaleRectHeight(AfViewUtil.scaleValue(mContext, scaleRectHeight));
        ...
        if(mSimpleSeriesRenderers!=null && mSimpleSeriesRenderers.length>0){
        	for(int i=0;i<mSimpleSeriesRenderers.length;i++){
        		...
        		mSimpleSeriesRenderer.setChartValuesTextSize(AfViewUtil.scaleTextValue(mContext, mChartValuesTextSize));
        	}
        }
    }
    
    int chartTitleTextSize = (int)mRenderer.getChartTitleTextSize();
    mRenderer.setChartTitleTextSize(AfViewUtil.scaleTextValue(mContext, chartTitleTextSize));
    //轴线上标签文字大小
    int mLabelsTextSize  = (int)mRenderer.getLabelsTextSize();
    mRenderer.setLabelsTextSize(AfViewUtil.scaleTextValue(mContext, mLabelsTextSize));
  	//说明文字大小
    int mLegendTextSize  =  (int)mRenderer.getLegendTextSize();
    mRenderer.setLegendTextSize(AfViewUtil.scaleTextValue(mContext, mLegendTextSize));
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mChart instanceof XYChart) {
	    ...
	    if(mXYMultipleSeriesRenderer.isScaleLineEnabled()){
	        ...
	        int scaleCircleRadius = mXYMultipleSeriesRenderer.getScaleCircleRadius();
	        //按分辨率转换
	        scaleTopPadding = AfViewUtil.scaleValue(mContext, scaleTopPadding);
	        scaleBottomPadding = AfViewUtil.scaleValue(mContext, scaleBottomPadding);
	        ...
	        if(mChart instanceof XYChart){
	          ...
	          if(minValue<5){
	              ...
	              if(showRect){
	                  ...
	                  //行高
	                  float hSize1 = AfGraphicUtil.getDesiredHeight(mTextPaint1);
	                  
	                  //行高
	                  float hSize2 = AfGraphicUtil.getDesiredHeight(mTextPaint2);
	                  
	                  int textPadding = 8;
	                  textPadding = AfViewUtil.scaleValue(mContext, textPadding);
	                  
	                  //设置个新的长方形  
	                  //判断文字是否超出设置的框框
	                  int row1 = AfGraphicUtil.getDrawRowCount(showValue,mXYMultipleSeriesRenderer.getScaleRectWidth()-10,mTextPaint1);
	                  int row2 = AfGraphicUtil.getDrawRowCount(showExplain,mXYMultipleSeriesRenderer.getScaleRectWidth()-10,mTextPaint2);
	                  RectF mRectF = null;
	                  ...
	                  AfGraphicUtil.drawText(canvas,showValue,mXYMultipleSeriesRenderer.getScaleRectWidth(),mTextPaint1,mRectLeft+textPadding, mRectTop+textPadding);
	                  AfGraphicUtil.drawText(canvas,showExplain,mXYMultipleSeriesRenderer.getScaleRectWidth(),mTextPaint2,mRectLeft+textPadding, (int)(mRectTop+textPadding+row1*hSize1));
