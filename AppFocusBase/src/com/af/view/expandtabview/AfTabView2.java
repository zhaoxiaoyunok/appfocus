package com.af.view.expandtabview;
...
public class AfTabView2 extends LinearLayout{
	
	private ListView listView1;
	private ListView listView2;
	
	private AfTabListAdapter listViewAdapter1;
	private AfTabListAdapter listViewAdapter2;
	
	private List<AfMenuItem> groups;
	private List<ArrayList<AfMenuItem>> childrens;
	
	/**当前显示的分类2*/
	private List<AfMenuItem> childrenItem = new ArrayList<AfMenuItem>();
	...
	public AfTabView2(Context context,List<AfMenuItem> groups,List<ArrayList<AfMenuItem>> childrens,int defaultGroupId,int defaultChildrenId,int selectGroupResId, int selectorGroupResId,int selectChildrenResId, int selectorChildrenResId) {
		super(context);
		this.groups = groups;
	...
	public AfTabView2(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init(Context context) {
		...
		listViewAdapter1 = new AfTabListAdapter(context, groups,selectGroupResId,selectorGroupResId);
		...
		listViewAdapter1.setOnItemClickListener(new AfTabListAdapter.OnItemClickListener() {
			@Override
	    ...
	    listViewAdapter2 = new AfTabListAdapter(context, childrenItem,selectChildrenResId,selectorChildrenResId);
	    listViewAdapter2.setSelectedPositionNoNotify(defaultChildrenPosition);
	    listView2.setAdapter(listViewAdapter2);
	    listViewAdapter2.setOnItemClickListener(new AfTabListAdapter.OnItemClickListener() {
