...
/**
 * The Class AfLevelChart.
 */
public class AfLevelChart extends AfLevelAbstractChart {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;
	
	/** The m dataset. */
	protected AfLevelSeriesDataset mDataset;
	
	/** The m renderer. */
	protected AfLevelSeriesRenderer mRenderer;
	 ...
	 */
	public AfLevelChart(AfLevelSeriesDataset mDataset,AfLevelSeriesRenderer mRenderer) {
		super();
	 ...
	 */
	protected void setDatasetRenderer(AfLevelSeriesDataset dataset,AfLevelSeriesRenderer renderer) {
		mDataset = dataset;
		mRenderer = renderer;
	}
	 ...
	 * @see com.af.view.level.AfLevelAbstractChart#draw(android.graphics.Canvas, int, int, int, int, int, int, android.graphics.Paint)
	 */
	public void draw(Canvas canvas, int x, int y,int measureWidth,int measureHeight,int screenWidth, int screenHeight,
		...
		textLevelSize = AfViewUtil.scale(screenWidth,screenHeight, textLevelSize);
		//当前值文字与顶部的距离
        ...
        int textWidth = (int)AfGraphicUtil.getStringWidth(textValue,mTextPaint);
		...
		for(int i=0;i<color.length;i++){
			...
			if(textlevelIndex == i){
		        ...
		        int textDescWidth = (int)AfGraphicUtil.getStringWidth(textDesc,mTextPaint);
			...
			if(partValue!=null && partValue.length == color.length){
		        ...
		        int partValueWidth = (int)AfGraphicUtil.getStringWidth(String.valueOf(partValue[i]),mTextPaint);
