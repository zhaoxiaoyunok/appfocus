...
import com.af.adapter.AfFragmentPagerAdapter;
import com.af.util.AfLogUtil;
import com.af.util.AfViewUtil;
import com.af.view.sample.AfHorizontalScrollView;
import com.af.view.sample.AfHorizontalScrollView.AbOnScrollListener;

// TODO: Auto-generated Javadoc

/**
 * © 2015 lyyj.com
 * 名称：AfSlidingSmoothTabView.java 
 ...
 * 
 */
public class AfSlidingSmoothTabView extends LinearLayout {
	...
	/** tab的线性布局父. */
	private AfHorizontalScrollView mTabScrollView  = null;
	...
	/** 内容区域的适配器. */
	private AfFragmentPagerAdapter mFragmentPagerAdapter = null;
	 ...
	 */
	public AfSlidingSmoothTabView(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		mTabScrollView  = new AfHorizontalScrollView(context);
		...
		if(!(this.context instanceof FragmentActivity)){
			AfLogUtil.e(AfSlidingSmoothTabView.class, "构造AbSlidingSmoothTabView的参数context,必须是FragmentActivity的实例。");
		}
		
		FragmentManager mFragmentManager = ((FragmentActivity)this.context).getSupportFragmentManager();
		mFragmentPagerAdapter = new AfFragmentPagerAdapter(
		...
		mTabScrollView.setOnScrollListener(new AbOnScrollListener() {
			...
			public void onScrollToRight() {
				AfLogUtil.d(AfSlidingSmoothTabView.class, "onScrollToRight");
			}
			
			@Override
			public void onScrollToLeft() {
				AfLogUtil.d(AfSlidingSmoothTabView.class, "onScrollToLeft");
			}
			
			@Override
			public void onScrollStoped() {
				AfLogUtil.d(AfSlidingSmoothTabView.class, "onScrollStoped");
			}
			...
			public void onScroll(int arg1) {
				...
				AfLogUtil.d(AfSlidingSmoothTabView.class, "滑动X"+startX+"to"+toX);
	...
	public void computeTabImg(int index){
		...
		AfViewUtil.measureView(tabView);
		...
		AfLogUtil.d(AfSlidingSmoothTabView.class, "old--startX:"+startX);
		//判断当前显示的是否被被屏幕遮挡，如果遮挡就移动（如果是第一个和最后一个移动遮挡的宽度否则移动tab的宽度）
		AfLogUtil.d(AfSlidingSmoothTabView.class, "view宽度"+index+":"+tabView.getMeasuredWidth());
		AfLogUtil.d(AfSlidingSmoothTabView.class, "ScrollView宽度:"+mTabScrollView.getWidth());
		AfLogUtil.d(AfSlidingSmoothTabView.class, "scrollX:"+scrollX);
		AfLogUtil.d(AfSlidingSmoothTabView.class, "tabView right:"+tabView.getRight());
		AfLogUtil.d(AfSlidingSmoothTabView.class, "tabView left:"+tabView.getLeft());
		
		if(mSelectedTabIndex<index && tabView.getRight()-scrollX > mTabScrollView.getWidth()){
			AfLogUtil.d(AfSlidingSmoothTabView.class, "右边被遮挡");
			int offsetX = 0;
			//右边被遮挡
			if(index == mTabLayout.getChildCount()-1){
				...
				AfLogUtil.d(AfSlidingSmoothTabView.class, "startX:"+startX+",offsetX:"+offsetX);
			...
			AfLogUtil.d(AfSlidingSmoothTabView.class, "左边被遮挡");
			//左边被遮挡  offsetX是负值
			int offsetX = 0;
            if(index == 0){
            	offsetX = -scrollX;
				...
				mTabScrollView.smoothScrollBy(offsetX, 0);
				scrollX = scrollX+offsetX;
				AfLogUtil.d(AfSlidingSmoothTabView.class, "startX2:"+startX+",offsetX:"+offsetX);
				int toX = tabView.getLeft()-scrollX;
				imageSlide(mTabImg,startX,toX,0,0);
				startX = toX;
			}
	...
	public void addItemView(String tabText,Fragment fragment){
		...
		//重新
		AfLogUtil.d(AfSlidingSmoothTabView.class, "addItemView finish");
		mFragmentPagerAdapter.notifyDataSetChanged();
		mViewPager.setCurrentItem(0);
		computeTabImg(0);
	}
