 ...
 * 
 */
public class AfPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private AfListViewHeader mHeaderView;
	
	/** footer view. */
	private AfListViewFooter mFooterView;
	 ...
	 */
	public AfPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public AfPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void addHeaderView() {
        mHeaderView = new AfListViewHeader(mContext);
	...
	private void addFooterView() {
        ...
        mFooterView = new AfListViewFooter(mContext);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != AfListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(AfListViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(AfListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != AfListViewFooter.STATE_LOADING) {
			mFooterView.setState(AfListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(AfListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(AfListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(AfListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(AfListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(AfListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(AfListViewFooter.STATE_READY);
		}
     ...
     */
    public AfListViewHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public AfListViewFooter getFooterView() {
        return mFooterView;
    }
