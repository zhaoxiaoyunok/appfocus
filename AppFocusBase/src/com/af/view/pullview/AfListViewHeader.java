 ...
 * 
 */
public class AfListViewHeader extends LinearLayout {
	 ...
	 */
	public AfListViewHeader(Context context) {
		super(context);
		initView(context);
	}
	 ...
	 */
	public AfListViewHeader(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView(context);
	}
	...
	private void initView(Context context) {
		...
		AfViewUtil.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage = AfFileUtil.getBitmapFromSrc("image/arrow.png");
		...
		layoutParamsWW.width = AfViewUtil.scaleValue(mContext, 50);
		layoutParamsWW.height = AfViewUtil.scaleValue(mContext, 50);
		...
		AfViewUtil.setPadding(headTextLayout,0, 0, 0, 0);
		...
		AfViewUtil.setTextSize(tipsTextview,30);
		AfViewUtil.setTextSize(headerTimeView,27);
		...
		layoutParamsWW3.rightMargin = AfViewUtil.scaleValue(mContext, 10);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = AfDateUtil.getCurrentDate(AfDateUtil.dateFormatHMS);
