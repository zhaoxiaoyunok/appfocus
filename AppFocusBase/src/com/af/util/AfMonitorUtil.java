...
import com.af.view.app.AfMonitorView;
 ...
 * 
 */
public final class AfMonitorUtil {
	
	/** 性能测试开关状态. */
	public static boolean mMonitorOpened = false;
	
	/** 帧测试View. */
	private static AfMonitorView mAbMonitorView  = null;
	...
	public static void openMonitor(Context context){
		...
		DisplayMetrics display = AfAppUtil.getDisplayMetrics(context);
		final int diaplayWidth = display.widthPixels;
		
		mAbMonitorView  = new AfMonitorView(context);
         ...
         */
        // 设置悬浮窗的长得宽
		mMonitorParams.width = AfViewUtil.scaleValue(context, 100);
		mMonitorParams.height = AfViewUtil.scaleValue(context, 50);
