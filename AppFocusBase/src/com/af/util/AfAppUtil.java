...
import com.af.model.AfAppProcessInfo;
import com.af.model.AfCPUInfo;
import com.af.model.AfProcessInfo;
import com.af.model.AfPsRow;
...
public class AfAppUtil {
     ...
     * @return
     */
    public static List<AfAppProcessInfo> getRunningAppProcesses(Context context) {
    	ActivityManager activityManager = null;
    	List<AfAppProcessInfo> list = null;
    	PackageManager packageManager = null;
	    try {
	    	activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
	    	packageManager = context.getApplicationContext().getPackageManager();
	    	list = new ArrayList<AfAppProcessInfo>();
	    	//所有运行的进程
	    	List<RunningAppProcessInfo> appProcessList = activityManager.getRunningAppProcesses();
	    	ApplicationInfo appInfo = null;
	    	AfAppProcessInfo abAppProcessInfo = null;
	    	...
	    	for(RunningAppProcessInfo appProcessInfo:appProcessList){
	    		abAppProcessInfo = new AfAppProcessInfo(appProcessInfo.processName,appProcessInfo.pid,appProcessInfo.uid);
	    		...
	    		AfProcessInfo processInfo = getMemInfo(appProcessInfo.processName);
	 ...
	 * 描述：执行PS.
	 */
    public static List<AfPsRow> ps() {
    	List<AfPsRow> psRowlist = new ArrayList<AfPsRow>();
        String ps = runScript("ps");
        String[] lines = ps.split("\n");
        psRowlist = new ArrayList<AfPsRow>();
        for (String line : lines) {
        	AfPsRow row = new AfPsRow(line);
     ...
     * @return
     */
    public static AfPsRow getPsRow(String processName) {
    	List<AfPsRow> psRowlist = ps();
        for (AfPsRow row : psRowlist) {
     ...
     * @return
     */
    public static AfProcessInfo getMemInfo(String processName) {
    	AfProcessInfo process = new AfProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
        ...
        if(process.memory == 0){
        	AfLogUtil.d(AfAppUtil.class, "##"+processName+",top -n 1未找到");
        }
        return process;
    }
     ...
     * @return
     */
    public static AfProcessInfo getMemInfo(int pid) {
    	AfProcessInfo process = new AfProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
	...
	public static String runCommand(String[] command, String workdirectory){
		String result = "";
		AfLogUtil.d(AfAppUtil.class, "#"+command);
		try {
	 ...
	 * @return
	 */
	public static AfCPUInfo getCPUInfo() {        
		AfCPUInfo CPUInfo = null;        
		try {            
			String result = runCommandTopN1();  
	 ...
	 * @return
	 */
	public static AfCPUInfo parseCPUInfo(String info) {
		 AfCPUInfo CPUInfo = new AfCPUInfo();
		 String tempString = "";
		 String[] rows = null;
		 String[] columns = null;
		 rows = info.split("[\n]+");
	...
	public static long getTotalMemory(Context context){  
        ...
        try{  
            ...
            for(String str:strs){  
                AfLogUtil.d(AfAppUtil.class,str+"\t");  
            }  
            //获得系统总内存，单位KB  
