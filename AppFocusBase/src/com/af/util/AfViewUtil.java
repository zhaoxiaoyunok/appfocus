...
import com.af.view.pullview.AfListViewFooter;
import com.af.view.pullview.AfListViewHeader;
...
public class AfViewUtil {
	...
	public static int scaleValue(Context context, float value) {
		DisplayMetrics mDisplayMetrics = AfAppUtil.getDisplayMetrics(context);
		//为了兼容尺寸小密度大的情况
		if(mDisplayMetrics.scaledDensity > AfAppConfig.UI_DENSITY){
			//密度
			if(mDisplayMetrics.widthPixels > AfAppConfig.UI_WIDTH){
				value = value*(1.3f - 1.0f/mDisplayMetrics.scaledDensity);
			}else if(mDisplayMetrics.widthPixels < AfAppConfig.UI_WIDTH){
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		float scale = 1;
		try {
			float scaleWidth = (float) displayWidth / AfAppConfig.UI_WIDTH;
			float scaleHeight = (float) displayHeight / AfAppConfig.UI_HEIGHT;
    ...
    public static void scaleContentView(ViewGroup contentView){
    	AfViewUtil.scaleView(contentView);
    ...
    public static boolean isNeedScale(View view){
    	if (view instanceof AfListViewHeader){
    		return false;
        }
    	
    	if (view instanceof AfListViewFooter){
    		return false;
        }
    	return true;
    }
    ...
    */
    public static void setViewSize(View view,int widthPixels, int heightPixels){
        ...
        if(params == null){
            AfLogUtil.e(AfViewUtil.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
